...
def mutual_discriminator(x, opt, reuse = False):
	...
	with tf.variable_scope("infogan_discriminator", reuse=reuse):
...
def con_generator(z, y, opt, reuse = False):
	...
	with tf.variable_scope("con_generator", reuse = reuse):
...
def con_discriminator(x, y, opt, reuse = False):
	...
	with tf.variable_scope("con_discriminator", reuse = reuse):
...
def vaes_encoder(x, opt, reuse = None):
	with tf.variable_scope("EncoderX2Z", reuse = reuse):
		mu, scale = encoder1(x, opt)
	return mu, scale

def vaes_decoder(z, opt, reuse = None):
	with tf.variable_scope("DecoderZ2X", reuse = reuse):
		h = decoder2(z, opt)
	return h
...
def infogan_discriminator(x, opt, reuse = None):
	with tf.variable_scope('InfoGANDiscriminator', reuse = reuse):
		f, q_out = mutual_discriminator(x, opt)
	return f, q_out

def michigan_generator(z, y, opt, reuse = None):
	with tf.variable_scope("MichiGANGenerator", reuse=reuse):
		h = con_generator(z, y, opt)
	return h

def michigan_discriminator(x, y, opt, reuse = None):
	with tf.variable_scope('MichiGANDiscriminator', reuse = reuse):
