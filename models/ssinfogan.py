...
class ssInfoWGAN_GP(BaseGAN):

	"""
	Semi-Supervised Information Maximizing Wasserstein GAN 
	with gradient penalty (ssInfoWGAN-GP)
	"""
	def __init__(self, x_dimension, z_dimension = 10, noise_dimension = 118, **kwargs):
		...
		self.lamb_mi_real = kwargs.get("lamb_mi_real", 1.0)
		...
		with tf.device(self.device):
			...
			self.y = tf.placeholder(tf.float32, shape = [None, self.z_dim], name = "latent_real")
			self.noise = tf.placeholder(tf.float32, shape = [None, self.n_dim], name = "latent_noise")
	...
	def generatorDropOut(self):
		"""
		generator with dropout layers of ssInfoWGAN-GP
		"""
	...
	def generator(self):
		"""
		generator without dropout layers of ssInfoWGAN-GP
		"""
	...
	def discriminatorInfo(self, x_input):
		"""
		discriminator of ssInfoWGAN-GP
		"""
	...
	def create_network(self):
		"""
		construct the ssInfoWGAN-GP networks
		"""
		...
		self.q_vector_real = self.c_mutual_info(self.c_mutual_real, self.y)
		self.q_real_mutual = tf.reduce_mean(self.q_vector_real)
	...
	def loss_function(self):
		...
		self.G_loss = tf.reduce_mean(self.Dx_fake) - (self.q_fake_mutual * self.lamb_mi_fake) - (self.q_real_mutual * self.lamb_mi_real) 
		
		self.gradient_penalty = self.compute_gp(self.x, self.x_gen_data, self.discriminatorInfo)
		self.D_loss = D_raw_loss + self.lamb_gp * self.gradient_penalty - (self.q_fake_mutual * self.lamb_mi_fake) - (self.q_real_mutual * self.lamb_mi_real) 
	...
	def train(self, train_data, train_cond, use_validation = False, valid_data = None, valid_cond = None, use_test_during_train = False, test_data = None,
		...
		"""
		train ssInfoWGAN-GP with train_data (AnnData), train_cond (numpy) and optional valid_data (numpy array) for n_epochs. 
		"""
		log.info("--- Training ---")
		...
		for epoch in range(1, n_epochs + 1):
			...
			for _ in range(1,  n_train // batch_size + 1):
				...
				for _ in range(self.Diters):
					x_mb, y_mb = self.sample_data_cond(train_data, train_cond, batch_size)
					z_mb = self.sample_z(batch_size, self.z_dim)
					n_mb = self.sample_z(batch_size, self.n_dim)
					self.sess.run(self.d_solver, feed_dict = {self.x: x_mb, self.y: y_mb, self.z: z_mb, self.noise: n_mb, 
				...
				_, current_loss_D, current_loss_G = self.sess.run([self.g_solver, self.D_loss, self.G_loss],
					feed_dict = {self.x: x_mb, self.y: y_mb, self.z: z_mb, self.noise: n_mb, self.is_training: self.if_BNTrainingMode})
			...
			if use_validation:
				for _ in range(1, n_valid // batch_size + 1):
					x_mb, y_mb = self.sample_data_cond(valid_data, valid_cond, batch_size)
					z_mb = self.sample_z(batch_size, self.z_dim)
					n_mb = self.sample_z(batch_size, self.n_dim)

					current_loss_valid_D, current_loss_valid_G = self.sess.run([self.D_loss, self.G_loss], 
						feed_dict = {self.x: x_mb, self.y: y_mb, self.z: z_mb, self.noise: n_mb, self.is_training: False})
	...
	def train_np(self, train_data, train_cond, use_validation = False, valid_data = None, valid_cond = None, use_test_during_train = False, test_data = None,
				 ...
				 early_stop_limit = 20, threshold = 0.0025, shuffle = True, save = False, model_save_path = None, output_save_path = None, verbose = False):
		"""
		train ssInfoWGAN-GP with train_data (numpy), train_cond (numpy) and optional valid_data (numpy array) for n_epochs. 
		"""
		log.info("--- Training ---")
		if use_validation and valid_data is None:
			raise Exception('valid_data is None but use_validation is True.')
		...
		for epoch in range(1, n_epochs + 1):
			...
			for _ in range(1,  n_train // batch_size + 1):
				...
				# D step
				for _ in range(self.Diters):
					x_mb, y_mb = self.sample_data_cond_np(train_data, train_cond, batch_size)
					z_mb = self.sample_z(batch_size, self.z_dim)
					n_mb = self.sample_z(batch_size, self.n_dim)
					self.sess.run(self.d_solver, feed_dict = {self.x: x_mb, self.z: z_mb, self.y: y_mb, 
															  self.noise: n_mb, self.is_training: self.if_BNTrainingMode})
				...
				n_mb = self.sample_z(batch_size, self.n_dim)
				_, current_loss_D, current_loss_G = self.sess.run([self.g_solver, self.D_loss, self.G_loss],
					feed_dict = {self.x: x_mb, self.y: y_mb, self.z: z_mb, self.noise: n_mb, 
								 self.is_training: self.if_BNTrainingMode})
			...
			if use_validation:
				for _ in range(1, n_valid // batch_size + 1):
					x_mb, y_mb = self.sample_data_cond_np(valid_data, valid_cond, batch_size)
					z_mb = self.sample_z(batch_size, self.z_dim)
					n_mb = self.sample_z(batch_size, self.n_dim)
